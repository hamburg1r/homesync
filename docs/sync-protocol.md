...
- **Tracking rules (phone):** named regex/folder/**file** rules in Settings (empty = no auto upload). Group name optional (default `misc`). Scanner walks granted storage roots for regex/folder; file rules target one absolute path. `source_kind` is inferred from path (`DCIM`→camera, WhatsApp→whatsapp, Download→download, else `misc`). Matches auto-ingest via the same queue. Tracked files waiting to upload show chip **`pending`**; failures show **`failed`** — not availability `listed`. List refreshes after index update so pending rows appear before uploads finish. BLAKE3 hashing runs on a background isolate; ingest progress UI is throttled and does not rebuild the file list every chunk.
